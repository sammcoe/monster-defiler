...
var RemotePlayer = function (index, game, player, startX, startY, startAngle) {
  ...
  this.player = MonsterDefiler.game.add.sprite(x, y, 'enemy')
  ...
  MonsterDefiler.game.physics.enable(this.player, Phaser.Physics.ARCADE)
...
RemotePlayer.prototype.update = function () {
  if (this.player.x !== this.lastPosition.x || this.player.y !== this.lastPosition.y || this.player.angle != this.lastPosition.angle) {
    ...
    this.player.rotation = Math.PI + MonsterDefiler.game.physics.arcade.angleToXY(this.player, this.lastPosition.x, this.lastPosition.y)
  } else {
